...
def read_alpha_sigma_or_set_defaults(path="output/blp_q8_newton_results.csv", alpha_default=-3.044808357870437, sigma_default=36.69195461195635):
...
def q9_elasticity_and_diversion(data_path="input/data_yoghurt.csv", center_mode="market", city=1, period=1, tabledir="tables"):
    ...
    el_out = os.path.join(tabledir, f"q9_elasticities_city{city}_period{period}.tex")
    dv_out = os.path.join(tabledir, f"q9_diversion_city{city}_period{period}.tex")
    export_df_to_latex(el_df, el_out, caption=f"Own- And Cross-Price Elasticities, City {city}, Period {period}", label=f"tab:q9_elasticities_city{city}_period{period}", index=False, use_booktabs=True)
    export_df_to_latex(dv_df, dv_out, caption=f"Diversion Ratios, City {city}, Period {period}", label=f"tab:q9_diversion_city{city}_period{period}", index=False, use_booktabs=True)
...
if __name__ == "__main__":
    q9_elasticity_and_diversion(data_path="input/data_yoghurt.csv", center_mode="market", city=1, period=1, tabledir="tables")
