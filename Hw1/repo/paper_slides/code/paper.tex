...
\;}
\]
This representation is linear in the parameters once $\log s_{0ct}$ is computed
from the observed outside share, and it highlights the IIA structure of the logit:
each equation involves only product $j$'s covariates and an unobserved
demand shock $\xi_{jct}$ (to be addressed with instruments when $p_{jct}$ is endogenous).
